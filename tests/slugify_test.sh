# tests/slugify_test.sh — lib/vault.sh:slugify, and parity with migite.paths.slugify
#
# `migite` (bash) names vault folders with lib/vault.sh's slugify; the standalone
# tools (Python) look them up with migite.paths.slugify. There used to be four
...
if "$MIGITE_PYTHON" -c 'import sys' &>/dev/null; then
  for input in "Add PDF Export" "foo_bar" "fix N+1 on /districts!" "BB-3370" "  --Hello World-- " "$long" \
    ...
    py=$("$MIGITE_PYTHON" -m migite.paths slugify "$input")
