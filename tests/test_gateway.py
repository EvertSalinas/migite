#!/usr/bin/env python3
"""tests/test_gateway.py - unittest coverage for gateway, the shared
...
class CallClaudeTest(_FakeClaude):
    def test_success_returns_text_and_usage(self):
        self.mode("envelope")
        r = gateway.call_agent("hello world", "knowledge", model="claude-sonnet-5", tool="t", label="l", ledger=self.ledger)
    ...
    def test_ledger_gets_one_line_per_call(self):
        self.mode("envelope")
        gateway.call_agent("a", "knowledge", model="m", tool="migite-plan", label="explore:models", ledger=self.ledger)
        gateway.call_agent("b", "knowledge", model="m", tool="migite-plan", label="synthesize_plan", ledger=self.ledger)
        recs = gateway.read_ledger(self.ledger)
    ...
    def test_ledger_env_var_is_honoured(self):
        ...
        gateway.call_agent("a", "knowledge", model="m")
        self.assertEqual(len(gateway.read_ledger(self.ledger)), 1)

    def test_no_ledger_is_a_noop(self):
        self.mode("envelope")
        gateway.call_agent("a", "knowledge", model="m")  # must not raise, must not write anywhere

    def test_structured_output_is_parsed(self):
        self.mode("structured")
        r = gateway.call_agent("x", "knowledge", model="m", schema={"type": "object"})
    ...
    def test_schema_is_passed_on_the_command_line(self):
        ...
        gateway.call_agent("x", "knowledge", model="m", schema={"type": "object", "required": ["verdict"]})
    ...
    def test_effort_flag_is_passed_and_skipped_for_haiku(self):
        ...
        argv_file = str(Path(self.tmp.name) / "argv")
        os.environ["FAKE_CLAUDE_ARGV"] = argv_file
        gateway.call_agent("x", "knowledge", model="claude-opus-5-5", effort="xhigh")
        self.assertIn("--effort xhigh", Path(argv_file).read_text())
        gateway.call_agent("x", "knowledge", model="claude-haiku-4-5-20251001", effort="xhigh")
        self.assertNotIn("--effort", Path(argv_file).read_text())   # Haiku rejects the flag
        gateway.call_agent("x", "knowledge", model="claude-sonnet-5", effort="none")
        self.assertNotIn("--effort", Path(argv_file).read_text())
        gateway.call_agent("x", "knowledge", model="claude-sonnet-5")
        self.assertNotIn("--effort", Path(argv_file).read_text())
    ...
    def test_role_effort_table_from_configure(self):
        ...
        argv_file = str(Path(self.tmp.name) / "argv")
        os.environ["FAKE_CLAUDE_ARGV"] = argv_file
        old = dict(gateway.ROLE_EFFORT)
        try:
            gateway.ROLE_EFFORT = {"critic": "max", "knowledge": None}
            gateway.call_agent("x", "critic", model="claude-opus-5-5")
            self.assertIn("--effort max", Path(argv_file).read_text())
            gateway.call_agent("x", "knowledge", model="claude-sonnet-5")
            self.assertNotIn("--effort", Path(argv_file).read_text())
            gateway.call_agent("x", "critic", model="claude-opus-5-5", effort="low")   # explicit beats table
            self.assertIn("--effort low", Path(argv_file).read_text())
        finally:
            gateway.ROLE_EFFORT = old

    def test_plain_text_stdout_falls_back_gracefully(self):
        self.mode("text")
        r = gateway.call_agent("hi", "knowledge", model="m", ledger=self.ledger)
        self.assertTrue(r.text.startswith("plain text result for: hi"))
        self.assertEqual(r.usage.input_tokens, 0)
        self.assertEqual(r.raw, {})
        self.assertEqual(len(gateway.read_ledger(self.ledger)), 1)  # still recorded, zero usage

    def test_is_error_envelope_raises(self):
        self.mode("is_error")
        with self.assertRaises(gateway.AgentError) as cm:
            gateway.call_agent("x", "knowledge", model="m", ledger=self.ledger)
        self.assertIn("Something went wrong", str(cm.exception))
        self.assertFalse(gateway.read_ledger(self.ledger)[0]["ok"])

    def test_nonzero_exit_raises(self):
        self.mode("exit1")
        with self.assertRaises(gateway.AgentError):
            gateway.call_agent("x", "knowledge", model="m")

    def test_timeout_raises_and_records(self):
        self.mode("timeout")
        with self.assertRaises(gateway.AgentError) as cm:
            gateway.call_agent("x", "knowledge", model="m", timeout=1, ledger=self.ledger)
...
class GatewayTest(_FakeClaude):
    ...
    def test_reset_restores_the_built_in_state(self):
        ...
        gateway.DEFAULT_PERMISSION = "auto"
        gateway.reset()
        self.assertEqual((gateway.AGENT.name, gateway.ROLE_EFFORT, gateway.DEFAULT_PERMISSION, gateway.CONFIG),
                         ("claude", {}, "none", None))
    ...
    def test_check_agent_reports_path_and_missing_cli(self):
        ...
        gateway.AGENT = agents.get("cursor", binary="no-such-agent-cli")
        self.assertEqual(gateway.check_agent()["path"], "")

class HelpersTest(unittest.TestCase):
    def test_severity_counts(self):
        self.assertEqual(gateway.severity_counts("🔴 a 🔴 b 🟡 c 🟢 d 🟢 e 🟢 f"),
    ...
    def test_get_field(self):
        ...
        self.assertEqual(gateway.get_field(obj, "a.b.1.c"), "x")
...
class CliTest(unittest.TestCase):
    ...
    def run_cli(self, *args, stdin=""):
        return subprocess.run([sys.executable, "-m", "migite.gateway", *args],
...
class MissingCliTest(unittest.TestCase):
    ...
    TOOLS = ("migite/tools/plan.py", "migite/tools/review.py", "migite/tools/audit.py",
             "migite/tools/blueprint.py", "migite/tools/explore.py", "migite/tools/pr_review.py")

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.ledger = str(Path(self.tmp.name) / "usage.jsonl")
        self._agent = gateway.AGENT

    def tearDown(self):
        gateway.AGENT = self._agent
        self.tmp.cleanup()

    def use_binary(self, binary):
        gateway.AGENT = agents.get("cursor", binary=binary)

    def test_a_missing_binary_raises_agent_error_and_is_recorded(self):
        self.use_binary("no-such-agent-cli")
        with self.assertRaises(gateway.AgentError) as cm:
            gateway.call_agent("hi", "knowledge", model="", label="probe", ledger=self.ledger)
    ...
    def test_require_cli_accepts_an_installed_binary(self):
        ...
        gateway.require_cli()   # must not raise

    def test_every_tool_checks_for_its_cli_after_loading_config(self):
        for tool in self.TOOLS:
            src = (ROOT / tool).read_text()
            configured = src.index("gateway.configure_from(cfg)")
            self.assertIn("gateway.require_cli()", src[configured:], f"{tool} never calls require_cli")
    ...
    def test_a_tool_stops_with_one_clear_error_when_the_cli_is_missing(self):
        ...
        r = subprocess.run([sys.executable, "-m", "migite.tools.audit", "--repo-root", self.tmp.name],
