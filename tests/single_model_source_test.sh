...
# (migite/agents/<name>.py). Everything else names a role: the tools pass roles to the
# gateway, bash passes roles to `python -m migite.agent_cli ask`, and the config resolves
...
offenders=$(grep -lE 'claude-(haiku|sonnet|opus|fable)-[0-9]' \
  "$REPO_ROOT"/bin/* "$REPO_ROOT"/lib/*.sh "$REPO_ROOT"/lib/phases/*.sh \
  "$REPO_ROOT"/migite/*.py "$REPO_ROOT"/migite/tools/*.py "$REPO_ROOT"/migite/trackers/*.py \
  "$REPO_ROOT"/migite/agents/base.py "$REPO_ROOT"/migite/agents/__init__.py \
  "$REPO_ROOT"/migite/agents/cursor.py "$REPO_ROOT"/migite/agents/opencode.py 2>/dev/null \
  | xargs -I{} basename {} | sort | tr '\n' ' ')
check "single model source: no Claude model id outside migite/agents/claude.py (found: ${offenders:-none})" \
  test -z "$offenders"

check "single model source: migite/agents/claude.py names the three tier defaults" \
  bash -c 'grep -q "\"fast\": \"claude-haiku" "$1" && grep -q "\"standard\": \"claude-sonnet" "$1" && grep -q "\"strong\": \"claude-opus" "$1"' _ "$REPO_ROOT/migite/agents/claude.py"
...
from migite import agents, config as c
