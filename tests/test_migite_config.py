...
class ConfigFilePathTest(_Isolated):
    ...
    def test_repo_default_and_existing(self):
        self.assertEqual(migite_config.config_file_path(self.repo), self.repo / ".migite.yml")
        self.write_repo({}, name=".migite.json")
        self.assertEqual(migite_config.config_file_path(self.repo), self.repo / ".migite.json")   # an existing file wins

    def test_user_default_and_existing(self):
        user_dir = self.home / ".config" / "migite"
        self.assertEqual(migite_config.config_file_path(self.repo, user=True), user_dir / "config.yml")
        self.write_user({}, name="config.json")
        self.assertEqual(migite_config.config_file_path(self.repo, user=True), user_dir / "config.json")
