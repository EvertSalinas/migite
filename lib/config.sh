...
_config_editor() {
  ...
  editor="$("$MIGITE_PYTHON" -m migite.config --repo-root "$repo_root" get ui.editor 2>/dev/null)" || editor=""
...
_config_edit() {
  ...
  local file
  file="$("$MIGITE_PYTHON" -m migite.config --repo-root "$repo_root" path ${scope[@]+"${scope[@]}"})" || return 1
  if [[ ! -f "$file" ]]; then
    echo "No config file at $file yet; writing the starter first."
    "$MIGITE_PYTHON" -m migite.config --repo-root "$repo_root" init ${scope[@]+"${scope[@]}"} || return 1
  fi
  local -a editor_cmd
  ...
  if "$MIGITE_PYTHON" -m migite.config --repo-root "$repo_root" validate; then
    return 0
  fi
...
run_config_command() {
  ...
  case "${1:-}" in
    ...
    --path|path)
      shift
      "$MIGITE_PYTHON" -m migite.config --repo-root "$repo_root" path "$@"
      ;;
    --init|init)
      shift
