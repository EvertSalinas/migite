...
| **Backend / agent** | The agent CLI migite drives: Claude Code (default), Cursor CLI, or OpenCode, chosen by `agent.backend`. One adapter per agent in `migite/agents/` owns that CLI's flags, output format, and model ids; capabilities an agent lacks degrade explicitly. |
| **Gateway** | `migite/gateway.py`: the one path to an agent. Resolves a role to a model and effort, applies capability fallbacks, starts the process, records usage. Bash reaches it through `migite/agent_cli.py`. |
