...
7. **Real-time and UI data exposure** - Turbo Stream broadcasts (`broadcasts_to`, `broadcast_*_to`, `turbo_stream_from`) on a stream name shared across users or accounts, so private records reach subscribers who can't otherwise see them; broadcasts fired from model callbacks inside a loop or a bulk update (one render and push per record)
